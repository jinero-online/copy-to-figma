/**
 * copy-to-figma — build SVG "boards" (color palettes, type scales, icon grids,
 * full styleguides) and copy them to the clipboard as SVG text. Figma pastes
 * SVG text as an editable frame: shapes stay vectors, text stays *text* and is
 * laid out in the real font when the viewer has it installed.
 *
 * Why SVG text and not Figma's native clipboard format? The native fig-kiwi
 * payload can carry rectangles fine, but text nodes need Figma's pre-computed
 * glyph layout (derivedTextData), which Figma will not recompute for pasted
 * payloads — pasted text stays invisible until manually touched. SVG import
 * runs Figma's own text layout, so everything renders on paste. That makes
 * clipboard SVG the most practical "no plugin required" channel into Figma.
 *
 * All builders return { svg, count } and are pure — no DOM needed — so they
 * also run in Node for tests. Only the copy* helpers touch navigator.clipboard.
 */
...
// Top-level comma split that respects parentheses — for multi-shadow strings
// like "0 1px 2px rgba(0,0,0,.2), 0 8px 24px rgba(0,0,0,.1)".
...
function frame(w, h, inner, bg = '#FFFFFF') {
    return `<svg xmlns="http://www.w3.org/2000/svg" width="${w}" height="${h}" viewBox="0 0 ${w} ${h}">`
        + `<rect width="${w}" height="${h}" rx="16" fill="${bg}"/>` + inner + '</svg>';
}

const title = (text, pad = 32) =>
    `<text x="${pad}" y="${pad + 24}" font-family="${UI_FONT}" font-size="20" font-weight="700" fill="${INK}">${esc(text)}</text>`;

// ── Colors ───────────────────────────────────────────────────────────────────

/**
 * Color swatch board, grouped into labeled sections.
 * groups: [{ label, items: [{ hex, label }] }]
 */
export function buildColorsSvg(groups) {
...
/**
 * Type-scale board: left column = tag + parameters (UI font), right card =
 * sample text in the REAL font/size/weight — Figma applies the family on paste
 * when the font is installed, otherwise the fallback stays.
 * styles: [{ tag, family, size, weight, lineHeight, letterSpacing, color, sample }]
 */
export function buildTypographySvg(styles) {
    ...
    for (const s of styles) {
        ...
        const disp = Math.max(12, Math.min(real, 56)); // cap display size; real size stays in params
        ...
        lines.push([s.size, (s.weight || '400') + '', s.lineHeight && s.lineHeight !== 'normal' ? 'LH ' + s.lineHeight : ''].filter(Boolean).join('  ·  '));
        const extra = [];
...
/**
 * Font list board: each family name rendered in that family.
 * items: [{ family, note }]
 */
export function buildFontsSvg(items) {
...
/**
 * Spacing scale board: label, a bar whose width equals the value, the px value.
 * items: [{ label, px }]
 */
export function buildSpacingSvg(items) {
...
/**
 * Border-radius board: a row of squares, each with the real corner radius.
 * values: [px, …]
 */
export function buildRadiusSvg(values) {
...
/**
 * Shadows board: cards carrying an feDropShadow approximation of the first
 * (non-inset) component, labelled with the full CSS value.
 * NOTE: Figma's SVG import may drop filters — the labelled cards still arrive.
 * shadows: ['0 2px 8px rgba(…)', …]
 */
export function buildShadowsSvg(shadows) {
    ...
    // The background <rect> must directly follow <svg> — board composition relies on it.
    const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="${frameW}" height="${frameH}" viewBox="0 0 ${frameW} ${frameH}">`
        + `<rect width="${frameW}" height="${frameH}" rx="16" fill="#FAFAFA"/><defs>${defs.join('')}</defs>${parts.join('')}</svg>`;
...
/**
 * Buttons board: each button as a rounded rect + centered label in its real
 * colors, radius, border, font size/weight and padding.
 * buttons: [{ text, bgColor, textColor, borderRadius, fontSize, fontWeight, padding, border }]
 */
export function buildButtonsSvg(buttons) {
    ...
    for (const { b, fs, text, w, h, rx } of rows) {
        ...
        const stroke = (b.border && b.border !== 'none' && borderM)
            ? ` stroke="${esc(borderM[2] || '#D0D0D0')}" stroke-width="${num(borderM[1], 1)}"`
            : '';
        ...
        const meta = [b.bgColor, b.textColor, b.borderRadius].filter(Boolean).join('  ·  ');
...
// Re-embed an icon's own <svg> as a positioned nested svg. Keeps the original
// viewBox (or synthesizes one from width/height) so scaling is automatic.
...
/**
 * Icon grid board — each icon pasted as editable vectors, with a name label.
 * currentColor icons inherit the board's ink color.
 * items: [{ name, svg }] (raw SVG markup each)
 */
export function buildIconsSvg(items) {
    ...
    const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="${frameW}" height="${frameH}" viewBox="0 0 ${frameW} ${frameH}" color="${INK}" style="color:${INK}">`
        + `<rect width="${frameW}" height="${frameH}" rx="16" fill="#FFFFFF"/>${parts.join('')}</svg>`;
    return { svg, count };
}

// ── Styleguide composition ───────────────────────────────────────────────────

// Strip a board's outer <svg> + background rect, keeping its inner nodes.
...
/**
 * Compose any combination of sections into ONE board.
 * sections: { colors?, typography?, fonts?, spacing?, radius?, shadows?, buttons?, icons? }
 * (each in its builder's input format)
 * @returns {{ svg: string, count: number }}
 */
export function buildStyleguideSvg({ colors, typography, fonts, spacing, radius, shadows, buttons, icons } = {}) {
    const boards = [];
    let count = 0;
    const add = (r, label = null) => { boards.push({ label, ...stripBoard(r.svg) }); count += r.count; };
    ...
    const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="${maxW}" height="${y - GAP + PAD}" viewBox="0 0 ${maxW} ${y - GAP + PAD}" color="${INK}" style="color:${INK}">`
        + `<rect width="${maxW}" height="${y - GAP + PAD}" fill="#F4F4F5"/>${parts.join('')}</svg>`;
    return { svg, count };
}
...
export const copyColorsToClipboard = (groups) => copySvg(buildColorsSvg(groups));
export const copyTypographyToClipboard = (styles) => copySvg(buildTypographySvg(styles));
export const copyIconsToClipboard = (items) => copySvg(buildIconsSvg(items));
export const copyStyleguideToClipboard = (sections) => copySvg(buildStyleguideSvg(sections));
